...
#
# def write_xls(values, col_names, row_names, path):
#     assert len(col_names) * len(row_names) == len(values) * len(values[0])
#
#     wb = xlwt.Workbook()
...
#
#     # write values
#     for row_idx, col_values in enumerate(values):
#         for col_idx, value in enumerate(col_values):
#             ws.write(row_idx + 1, col_idx + 1, value)
#
#     # save
#     wb.save(path)
